...
parser.add_argument("-init", "--initialize_code", help="Initialize component code", action="store_true")
...
parser.add_argument("-l", "--location", help="Location to user folder")
args = parser.parse_args()

# Create manage user api
component = wings.ManageComponent(args.server, args.userid, args.domain, args.location)
...
if component.login(args.password): 
	if args.new:
		if args.component_id:
			...
			component.add_component_type(args.component_type, args.parent_type)
		...
		with open(args.save_component) as dfile:    
			...
			component.initialize_component_code(args.component_id, args.language_code)
